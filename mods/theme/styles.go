...
var (
	...
	Title = lipgloss.NewStyle().Foreground(Pink).Background(Dark).Bold(true)
	// Logo is the ∅ mark that opens every frame header.
	Logo = lipgloss.NewStyle().Foreground(Green).Background(Dark).Bold(true)
	...
	// Border is the standard mod window frame.
